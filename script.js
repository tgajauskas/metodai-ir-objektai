...
const genderMale = data.filter(item => item.gender === 'Male').length
console.log("Vyrų skaičius:", genderMale)

const cars = data.filter(item => item.car_year > 2000)
const idForCars = cars.map(person => person.id)
console.log("Žmonių ID, kurių automobiliai naujesni negu 2000 metų: ", idForCars)

const sizes = data.filter(item => item.shirt_size === 'XS' || item.shirt_size === 'S')
const people = sizes.sort((a, b) => a.first_name.localeCompare(b.first_name))
console.log(people)